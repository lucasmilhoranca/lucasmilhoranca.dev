...
- `notify` job (GitHub-hosted): posts the outcome to a Discord channel via a channel Webhook (an embed: status, commit link, author, run log). Runs with `if: always()`, so a failed build or deploy is announced too, and `continue-on-error: true`, so a broken webhook never fails an otherwise-good deploy. Skips itself silently when the secret is absent.
- Only one GitHub Secret is used: `DISCORD_WEBHOOK_URL` (the Discord notification). The deploy itself needs none — it runs on the owner's own machine.
